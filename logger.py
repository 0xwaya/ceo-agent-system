...
class AgentLogger:
    ...
    def log_execution_start(self, task_description: str):
        ...
        self.logger.info(
            f"[{self.agent_type.upper()}] Starting execution: {task_description}"
        )
    ...
    def log_execution_error(self, error: str):
        ...
        self.logger.error(
            f"[{self.agent_type.upper()}] Execution failed: {error}"
        )
    
    def log_deliverable(self, deliverable: str):
        """Log deliverable creation"""
        self.logger.info(
            f"[{self.agent_type.upper()}] Deliverable: {deliverable}"
        )
    ...
    def log_guard_rail_violation(self, violation: str):
        ...
        self.logger.warning(
            f"[{self.agent_type.upper()}] Guard rail violation: {violation}"
        )
...
class APILogger:
    ...
    def log_response(self, endpoint: str, status_code: int, duration: float):
        """Log API response"""
        self.logger.info(
            f"{endpoint} -> {status_code} ({duration:.3f}s)"
        )
...
class SecurityLogger:
    ...
    def log_authorization_failure(self, username: str, resource: str):
        ...
        self.logger.warning(
            f"Authorization denied for user '{username}' on resource '{resource}'"
        )
    ...
    def log_suspicious_activity(self, description: str, client_ip: str):
        ...
        self.logger.warning(
            f"Suspicious activity from {client_ip}: {description}"
        )
...
class PerformanceLogger:
    ...
    def log_slow_query(self, query: str, duration: float, threshold: float = 1.0):
        ...
        if duration > threshold:
            self.logger.warning(
                f"Slow query ({duration:.2f}s > {threshold:.2f}s): {query}"
            )
