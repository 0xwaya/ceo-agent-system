...
class InsufficientBudgetError(BudgetError):
    ...
    def __init__(
        self, required: float, available: float, agent_type: Optional[str] = None, **kwargs
    ):
...
class DependencyError(OrchestrationError):
    ...
    def __init__(self, task_id: str, missing_dependencies: list, **kwargs):
        details = {"task_id": task_id, "missing_dependencies": missing_dependencies}
...
def handle_exception(
    exception: Exception, context: Optional[str] = None, log_func: Optional[callable] = None
) -> Dict[str, Any]:
    """
...
def raise_for_status(condition: bool, exception_class: type, message: str, **kwargs):
    """
