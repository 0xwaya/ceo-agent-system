...
class ReportService:
    ...
    def _analyze_company_overview(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        company_name = company_info.get("name") or company_info.get("company_name", "Not specified")
        return {
            ...
            "burn_rate": company_info.get("monthly_expenses", 0)
            - company_info.get("monthly_revenue", 0),
        ...
        return {
            "30_day_plan": {
                "period": (
                    ...
                    f"{(datetime.now() + timedelta(days=30)).strftime('%b %d, %Y')}"
                ),
            ...
            "60_day_plan": {
                "period": (
                    f"{(datetime.now() + timedelta(days=31)).strftime('%b %d')} - "
                    f"{(datetime.now() + timedelta(days=60)).strftime('%b %d, %Y')}"
                ),
            ...
            "90_day_plan": {
                "period": (
                    f"{(datetime.now() + timedelta(days=61)).strftime('%b %d')} - "
                    f"{(datetime.now() + timedelta(days=90)).strftime('%b %d, %Y')}"
                ),
...
The company is currently in the {company_info.get('stage', 'early')} stage with
significant growth potential but facing execution challenges.
Our primary focus must be on achieving product-market fit while maintaining financial discipline.

**Key Observations:**
- Market opportunity is substantial but competitive pressure increasing
- Current burn rate of ${burn_rate:,.0f}/month requires immediate attention
...
Current product is functional but has technical debt that will impede scaling.
Core architecture is sound but needs refactoring for performance and maintainability.
...
Product has solid foundation but user friction points preventing optimal engagement.
Design system needs standardization and mobile experience requires attention.
...
Basic legal structure in place but gaps exist in contracts, IP protection, and
regulatory compliance. Need to establish robust legal framework before scaling.
...
Need systematic approach to competitive intelligence, industry trends, and best
practices. Current research is ad-hoc and not driving strategic decisions.
...
"""
    ...
    def _assess_operations(self, company_info: Dict[str, Any]) -> str:
        ...
        return (
            "Operational processes are functional but manual and not scalable. "
            "Need automation and systematization before 3x growth."
        )

    def _assess_market_position(self, company_info: Dict[str, Any]) -> str:
        """Assess market position"""
        return (
            f"Emerging player in {company_info.get('industry', 'technology')} market "
            "with differentiated AI-powered approach. Market opportunity is large "
    ...
    def _assess_product_maturity(self, company_info: Dict[str, Any]) -> str:
        ...
        return (
            "Product is MVP stage with core functionality proven. Feature set "
            "sufficient for early adopters but needs expansion for mainstream market."
        )

    def _assess_team(self, company_info: Dict[str, Any]) -> str:
        """Assess team capability"""
        return (
            f"Team of {company_info.get('team_size', 'small')} is talented but "
            "stretched thin. Need strategic hires in sales, product, and operations "
            "to scale."
        )
    ...
    def _assess_technology(self, company_info: Dict[str, Any]) -> str:
        ...
        return (
            "Modern tech stack with Python, LangGraph, and AI models. Architecture "
            "is sound but needs refactoring for scale and performance."
        )

    def _assess_customers(self, company_info: Dict[str, Any]) -> str:
        """Assess customer traction"""
        return (
            "Early customers showing strong engagement and satisfaction. Need to "
            "scale acquisition through repeatable channels and improve retention metrics."
        )

    def _assess_competition(self, company_info: Dict[str, Any]) -> str:
        """Assess competitive landscape"""
        return (
            "Competitive landscape includes established players and emerging startups. "
            "Differentiation through AI capabilities and executive focus is strong "
            "but requires continuous innovation."
        )
    ...
    def _analyze_financials(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "expense_efficiency": "Needs optimization"
            if monthly_expenses > monthly_revenue * 2
            else "Reasonable",
            "cash_position": f"${company_info.get('available_cash', 100000):,.0f} available",
            "burn_multiple": round(monthly_expenses / max(monthly_revenue, 1), 2)
    ...
    def _assess_funding_needs(self, company_info: Dict[str, Any]) -> Dict[str, Any]:
        ...
        burn_rate = company_info.get("monthly_expenses", 50000) - company_info.get(
        ...
        return {
            ...
            "fundraising_timeline": "90-120 days for Series A"
            if company_info.get("stage") == "Early Stage"
            else "60-90 days for bridge round",
        }
    ...
    def _evaluate_agent_readiness(self) -> List[Dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                "agent": "Software Engineering Agent",
                "readiness": "70%",
                "status": "Additional training needed",
            },
    ...
    def _list_evaluated_tools(self) -> List[Dict[str, str]]:
        ...
        return [
            {
                "tool": "LangGraph 0.6",
                "verdict": "Adopted - excellent for agent orchestration",
            },
            ...
            {"tool": "Anthropic Claude", "verdict": "Adopted - excellent for analysis"},
            {
                ...
                "verdict": "Evaluating - Pinecone vs Weaviate",
            },
            {
                "tool": "Observability Tools",
                "verdict": "Evaluating - LangSmith vs Weights & Biases",
            },
        ]
