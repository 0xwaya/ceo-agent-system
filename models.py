    ...
    def create_default(cls) -> "BudgetTracker":
        ...
        allocations = {
            AgentType.LEGAL: BudgetAllocation(
                agent_type=AgentType.LEGAL, allocated=BudgetConfig.LEGAL_BUDGET
            ),
            AgentType.BRANDING: BudgetAllocation(
                agent_type=AgentType.BRANDING, allocated=BudgetConfig.BRANDING_BUDGET
            ),
            AgentType.WEB_DEV: BudgetAllocation(
                agent_type=AgentType.WEB_DEV, allocated=BudgetConfig.WEB_DEV_BUDGET
            ),
            AgentType.MARTECH: BudgetAllocation(
                agent_type=AgentType.MARTECH, allocated=BudgetConfig.MARTECH_BUDGET
            ),
            AgentType.CONTENT: BudgetAllocation(
                agent_type=AgentType.CONTENT, allocated=BudgetConfig.CONTENT_BUDGET
            ),
            AgentType.CAMPAIGNS: BudgetAllocation(
                agent_type=AgentType.CAMPAIGNS, allocated=BudgetConfig.CAMPAIGNS_BUDGET
            ),
        }
...
class OrchestrationState(BaseState):
    ...
    def get_completed_agents(self) -> List[AgentType]:
        ...
        return [agent_type for agent_type, result in self.agent_results.items() if result.success]

    def get_failed_agents(self) -> List[AgentType]:
        """Get list of failed agents"""
        return [
            agent_type for agent_type, result in self.agent_results.items() if not result.success
        ]
