...
class AgentGuardRail:
    ...
    def get_payment_approval_workflow(self, payment_type: PaymentType) -> Optional[PaymentApprovalWorkflow]:
    ...
    def enforce_execution_model(self, state: Dict) -> Dict:
        ...
        state["required_frameworks"] = self.quality_standard.frameworks if self.quality_standard else []
        
        # 4. Add budget constraints
        state["max_budget"] = self.budget_constraint.max_budget if self.budget_constraint else 0
        state["allowed_spend_categories"] = self.budget_constraint.allowed_categories if self.budget_constraint else []
...
if __name__ == "__main__":
    ...
    try:
        branding_guard.validate_execution_mode("Design 4 logo concepts using Golden Ratio principles")
    ...
    try:
        branding_guard.validate_budget_spend("Design software subscription (Adobe CC, Figma)", 120.0)
