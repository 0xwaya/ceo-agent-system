...
def analyze_strategic_objectives(state: CFOAgentState) -> dict:
    ...
    print(
        f"  Remaining Reserve: ${total_budget - total_estimated:,.0f} ({((total_budget - total_estimated) / total_budget * 100):.1f}%)"
    )
...
def deploy_specialized_agents(state: CFOAgentState) -> dict:
    ...
    for expertise in priority_order:
        ...
        try:
            ...
            print(
                f"  Knowledge Base: {len(agent.knowledge_base.key_principles)} principles, "
...
if __name__ == "__main__":
    ...
    print(
        f"Budget Utilized: ${sum(o['budget_used'] for o in final_state['agent_outputs']):,.0f} of ${final_state['total_budget']:,.0f}"
    )
