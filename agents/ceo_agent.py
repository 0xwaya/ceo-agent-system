...
def analyze_strategic_objectives(state: CEOAgentState) -> dict:
    ...
    if total_budget_required > state.get('total_budget', 0):
        print(f"  ⚠️  WARNING: Budget shortfall of ${total_budget_required - state.get('total_budget', 0):,.0f}")
        print(f"  🎯 CEO Decision: Prioritize critical tasks, seek additional funding or scope reduction")
    ...
    api_budget = sum(t['estimated_budget'] for t in identified_tasks if not t.get('requires_payment'))
    service_budget = sum(t['estimated_budget'] for t in identified_tasks if t.get('requires_payment'))
...
def synthesize_executive_report(state: CEOAgentState) -> dict:
    ...
    pending_approvals = len([a for a in state.get('pending_approvals', []) 
