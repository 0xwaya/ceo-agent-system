...
from models import (
    AgentType, Task, TaskStatus, AgentExecutionResult,
    BudgetAllocation, CompanyInfo
)
from exceptions import (
    AgentExecutionError, InsufficientBudgetError,
    DeliverableError, GuardRailViolationError
)
        ...
        try:
            ...
            if not self._deliverables:
                raise DeliverableError(
                    f"Agent {self.agent_type.value} produced no deliverables"
                )
            
            # Calculate execution time and cost
            execution_time = time.time() - self._execution_start_time
            total_cost = sum(
                task.actual_cost for task in (tasks or [])
        ...
        try:
            ...
            if 'cost' in task_result:
                ...
                if self.budget_allocation:
                    ...
                    self.logger.log_budget_usage(
                        actual_cost,
                        self.budget_allocation.remaining
                    )
            else:
                task.mark_completed()
